...
public class PaymentsController {
    ...
    @CircuitBreaker(name = "default", fallbackMethod = "createDepositPaymentFallback")
    ...
    @CircuitBreaker(name = "default", fallbackMethod = "createSubscriptionPaymentFallback")
        ...
        try {
            ...
            if (payment.getState().equals("approved")) {
                ...
                if (timestamp == null || timestamp.isBefore(LocalDateTime.now().minusSeconds(20))) {
                    ...
                    throw new IllegalStateException("Payment link has expired");
                }
    ...
    @GetMapping("/upgrade/error")
    public String upgradeError() {
        log.info("Upgrade cancel callback received");
    ...
    @GetMapping("/status")
    @CircuitBreaker(name = "default", fallbackMethod = "getPaymentStatusFallback")
    ...
    public ResponseEntity<Map<String, String>> getPaymentStatusFallback(String userId, Throwable t) {
        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).body(Map.of("error", "Service is currently unavailable. Please try again later."));
    }

    @GetMapping("/suscriptions/{userId}")
    @CircuitBreaker(name = "default", fallbackMethod = "getSuscriptionsByUserIdFallback")
